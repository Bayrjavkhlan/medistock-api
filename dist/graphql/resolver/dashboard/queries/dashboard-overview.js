...
const toAddress = (address) => [address?.address1, address?.address2, address?.province]
    .filter(Boolean)
    .join(", ");
const safeDate = (value) => (value ? value.toISOString() : null);
const toMapPayload = (hospitals, drugstores) => ({
...
const buildAdminOverview = async (ctx) => {
    const months = getLastMonths(6);
    const [totalHospitals, totalPharmacies, totalDrugs, totalEquipment, totalStaff, totalLogs, hospitals, pharmacies, recentHospitalRows, recentPharmacyRows, recentDrugRows, recentLogRows, topPharmacyRows, topHospitalRows, lowStockRows, availableCount, lowCount, outCount, unknownCount, hospitalMissingLocationCount, pharmacyMissingLocationCount, pharmacyWithoutInventoryCount,] = await Promise.all([
        ...
        ctx.prisma.pharmacyDrug.findMany({
            ...
            where: {
                OR: [
                    { quantity: { lte: 10 } },
                    {
                        status: { in: [client_1.InventoryStatus.LOW, client_1.InventoryStatus.OUT_OF_STOCK] },
                    },
                ],
            },
        ...
        ctx.prisma.pharmacyDrug.count({
            where: { status: client_1.InventoryStatus.AVAILABLE },
        }),
        ctx.prisma.pharmacyDrug.count({ where: { status: client_1.InventoryStatus.LOW } }),
        ctx.prisma.pharmacyDrug.count({
            where: { status: client_1.InventoryStatus.OUT_OF_STOCK },
        }),
        ctx.prisma.pharmacyDrug.count({
            where: { status: client_1.InventoryStatus.UNKNOWN },
        }),
        ctx.prisma.hospital.count({
            where: {
                organization: {
    ...
    const growthSeries = await Promise.all([
        {
            ...
            label: "Эмнэлэг",
            color: "#0f766e",
            count: (range) => ctx.prisma.hospital.count({
                where: {
                    createdAt: {
                        gte: range.start,
                        lt: range.end,
        ...
        },
        {
            ...
            label: "Эмийн сан",
            color: "#0284c7",
            count: (range) => ctx.prisma.pharmacy.count({
                where: {
                    createdAt: {
                        gte: range.start,
                        lt: range.end,
        ...
        },
        {
            ...
            label: "Эм",
            color: "#7c3aed",
            count: (range) => ctx.prisma.drug.count({
                where: {
                    createdAt: {
                        gte: range.start,
                        lt: range.end,
        ...
        },
        {
            ...
            label: "Лог",
            color: "#f97316",
            count: (range) => ctx.prisma.equipmentLog.count({
                where: {
                    createdAt: {
                        gte: range.start,
                        lt: range.end,
        ...
        label: series.label,
        color: series.color,
        points: await Promise.all(months.map(async (range) => ({
            label: range.label,
            value: await series.count(range),
        }))),
    })));
    const recentItems = [
        ...recentHospitalRows.map((item) => ({
            id: `hospital-${item.id}`,
    ...
    return {
        ...
        alerts: [
            ...
            },
            {
                ...
                severity: hospitalMissingLocationCount + pharmacyMissingLocationCount > 0
                    ? "info"
                    : "success",
            },
            {
                ...
                description: pharmacyWithoutInventoryCount > 0
...
const buildHospitalOverview = async (ctx) => {
    ...
    const equipmentStates = [
        {
            label: "Боломжтой",
            value: hospital.equipments.filter((item) => item.state === client_1.EquipmentState.AVAILABLE).length,
            helper: "Ашиглах боломжтой төхөөрөмж",
            tone: "success",
        },
        {
            label: "Хуваарилагдсан",
            value: hospital.equipments.filter((item) => item.state === client_1.EquipmentState.ASSIGNED).length,
            helper: "Хэрэглэгчид оноогдсон төхөөрөмж",
            tone: "teal",
        },
        {
            label: "Засвартай",
            value: hospital.equipments.filter((item) => item.state === client_1.EquipmentState.IN_MAINTENANCE).length,
            helper: "Засварын явцад байгаа төхөөрөмж",
            tone: "warning",
        },
        {
            label: "Эвдрэлтэй",
            value: hospital.equipments.filter((item) => item.state === client_1.EquipmentState.OUT_OF_ORDER).length,
            helper: "Яаралтай анхаарах төхөөрөмж",
            tone: "danger",
        },
    ];
    const maintenanceAndBrokenCount = (equipmentStates[2]?.value ?? 0) + (equipmentStates[3]?.value ?? 0);
    const activitySeries = await Promise.all([
        {
            key: "equipment",
            label: "Төхөөрөмж",
            color: "#0f766e",
            count: (range) => ctx.prisma.equipment.count({
                where: {
        ...
        },
        {
            ...
            label: "Захиалга",
            color: "#0284c7",
            count: (range) => ctx.prisma.booking.count({
                where: {
                    hospital: { organizationId },
                    createdAt: { gte: range.start, lt: range.end },
                },
    ...
    const nearbyPharmacies = pharmacies
        ...
        const canMeasure = hospitalAddress?.latitude != null &&
            hospitalAddress.longitude != null &&
            pharmacyAddress?.latitude != null &&
            pharmacyAddress.longitude != null;
        const distanceKm = canMeasure
            ? haversineDistance(hospitalAddress.latitude, hospitalAddress.longitude, pharmacyAddress.latitude, pharmacyAddress.longitude)
            : null;
        return {
            item,
            distanceKm,
        ...
        if (distanceKm != null)
            return true;
        return (item.organization.address?.province != null &&
    ...
    return {
        ...
        stats: [
            ...
            },
            {
                ...
                value: hospital.bookings.filter((item) => item.status === "PENDING").length,
        ...
        alerts: [
            {
                ...
                description: maintenanceAndBrokenCount > 0
            ...
            },
            {
                ...
                description: hospital.organization.address?.latitude != null &&
                ...
                severity: hospital.organization.address?.latitude != null &&
                    hospital.organization.address?.longitude != null
                    ? "success"
                    : "info",
            },
...
const buildPharmacyOverview = async (ctx) => {
    const organizationId = ctx.activeOrg?.organization.id;
    if (!organizationId)
        return null;
    ...
    if (!pharmacy)
        return null;
    const months = getLastMonths(6);
    const lowStockItems = pharmacy.inventory.filter((item) => item.quantity <= 10 ||
        item.status === client_1.InventoryStatus.LOW ||
        item.status === client_1.InventoryStatus.OUT_OF_STOCK);
    const activitySeries = await Promise.all([
        {
            key: "updates",
            label: "Нөөцийн шинэчлэлт",
            color: "#0284c7",
            count: (range) => ctx.prisma.pharmacyDrug.count({
                where: {
            ...
            count: (range) => ctx.prisma.pharmacyDrug.count({
                where: {
                    ...
                    OR: [
                        ...
                        {
                            status: {
                                in: [client_1.InventoryStatus.LOW, client_1.InventoryStatus.OUT_OF_STOCK],
                            },
                        },
                    ],
                },
    ...
    return {
        ...
        stats: [
            ...
            },
            {
                ...
                value: pharmacy.inventory.filter((item) => item.status === client_1.InventoryStatus.AVAILABLE).length,
                helper: "Шууд худалдаалах боломжтой listing",
                tone: "success",
            },
            {
                label: "Нөөц багатай",
                value: pharmacy.inventory.filter((item) => item.status === client_1.InventoryStatus.LOW || item.quantity <= 10).length,
                helper: "Анхаарал шаардсан listing",
                tone: "warning",
            },
            {
                label: "Дууссан",
                value: pharmacy.inventory.filter((item) => item.status === client_1.InventoryStatus.OUT_OF_STOCK || item.quantity === 0).length,
                helper: "Одоогоор олдохгүй listing",
                tone: "danger",
            },
            {
        ...
        inventoryStatus: [
            ...
            {
                label: "Нөөц багатай",
                value: pharmacy.inventory.filter((item) => item.status === client_1.InventoryStatus.LOW).length,
                helper: "Шуурхай шинэчлэх шаардлагатай",
                tone: "warning",
            },
            {
                label: "Дууссан",
                value: pharmacy.inventory.filter((item) => item.status === client_1.InventoryStatus.OUT_OF_STOCK).length,
                helper: "Нийлүүлэлт шаардлагатай",
                tone: "danger",
            },
            {
                label: "Тодорхойгүй",
                value: pharmacy.inventory.filter((item) => item.status === client_1.InventoryStatus.UNKNOWN).length,
        ...
        recentUpdates: pharmacy.inventory.slice(0, 6).map((item) => ({
            id: item.id,
            title: item.drug.name,
        ...
        alerts: [
            ...
            },
            {
                id: "location",
                title: "Байршлын мэдээлэл",
                description: pharmacy.organization.address?.latitude != null &&
                ...
                severity: pharmacy.organization.address?.latitude != null &&
                    pharmacy.organization.address?.longitude != null
                    ? "success"
                    : "info",
            },
...
exports.DashboardOverview = (0, nexus_1.queryField)("dashboardOverview", {
    type: types_1.DashboardOverviewObjectType,
    resolve: async (_parent, _args, ctx) => {
        ...
        if (activeOrg?.organization.type === client_1.OrganizationType.HOSPITAL) {
            return {
                role: "HOSPITAL",
                admin: null,
        ...
        if (activeOrg?.organization.type === client_1.OrganizationType.PHARMACY) {
            return {
                role: "PHARMACY",
                admin: null,
                hospital: null,
